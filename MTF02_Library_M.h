...
// Version 3.1 - Configured for 100Hz operation with FLOW_SCALE=100 and Metre units
...
void mtf02_get_velocity(float* vel_x, float* vel_y, float* vel_z) {
    if (vel_x) *vel_x = mtf02_get_velocity_x();
    if (vel_y) *vel_y = mtf02_get_velocity_y();
    if (vel_z) *vel_z = mtf02_get_velocity_z();
}
...
void mtf02_print_status() {
    Serial.printf("MTF02[100Hz]: Alt=%.2fm FlowQ=%d VelX=%.3f VelY=%.3f PosX=%.2f PosY=%.2f Ready=%s Rate=%.1fHz\n",
...
void mtf02_print_debug() {
    Serial.printf("MTF02 DEBUG: Dist=%.2fm FlowX=%d FlowY=%d Quality=%d VelX=%.3f VelY=%.3f\n",
